...
namespace dotnetapp.Services
{
    public class WorkshopEventService
    {
        ...
        public async Task<bool> DeleteWorkshopEvent(int workshopEventId)
        {
            ...
            bool isReferenced = await _context.Bookings.AnyAsync(b => b.WorkshopEventId == workshopEventId);
            if (isReferenced)
        ...
        public async Task<IEnumerable<WorkshopEvent>> GetEventsByOrganizer(string organizerName)
        {
            return await _context.WorkshopEvents
                .Where(e => e.OrganizerName.ToLower() == organizerName.ToLower())
                .ToListAsync();
        }
    }
}
